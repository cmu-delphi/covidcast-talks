...
date: "<br> ![](delphi.png) ![](cmu.png) <br><br> January 27, 2021"
footer: "Get the slides at: cmu-delphi.github.io/covidcast-talks/google.html"
output: 
  slidy_presentation:
    theme: cerulean
    highlight: tango
...
- We've gone from 7 members to over 50, to support the US COVID-19 response
...
I can't cover all of this! This will be a very nontechnical talk, focused on providing a basic understanding of the data and our project overall. (Please ask about `r col("forecasting")`, `r col("nowcasting")`, or anything else, during the Q & A!) Outline:

1. Demo of COVIDcast API and indicators
2. COVID-19 survey and medical claims data
3. Wrapping up, looking towards the future

`r col("Reproducible talk:")` all code included. <https://cmu-delphi.github.io/covidcast/talks/google/talk.html>
...
plot(cases, title = "Total COVID-19 cases per 100,000 people", 
     choro_params = list(subtitle = format.Date(end_day, "%B %d %Y"), 
                         legend_n = 6), range = c(0, 1e4))
```
...
sympt = covidcast_signal(data_source = "fb-survey", 
                         ...
                         start_day = "2020-04-16", end_day = end_day,
...
plot(vaccine, title = "% of people who would accept COVID-19 vaccine",  
     choro_col = c("#D9F0C2", "#BFE6B5", "#5585B1", "#1F589F"),
     choro_params = list(subtitle = format.Date(end_day, "%B %d %Y"),
                         legend_n = 6), range = c(55, 95))
```


# Data Revisions 

By default the API returns the most recent data, but we also provide access to all previous versions of the data. Why would we need this? Because many data sources are subject to `r col("revisions:")`
...
# Massive COVID-19 Survey

Through recruitment partnership with Facebook, we survey over `r col("50,000 people daily")` (and over 16 million since it began in April), in the US

- We construct numerous (over 20) daily, county-level indicators from the survey
- Several are motivated by relevance to forecasting and nowcasting. For example: `r col("% CLI")` = percentage of people with COVID-like illness, or `r col("% CLI-in-community")` = percentage of people who know someone with CLI
- Many others are motivated by broader perspective in public health: understanding how the pandemic is `r col("affecting people")` (topics include mental health, social contacts, and behavior, broken down by various demographics)

A [parallel, international effort](https://covidmap.umd.edu/) by the University of Maryland reaches 100+ countries in 55 languages. Raw response data is freely available to researchers who sign a [data use agreement](https://dataforgood.fb.com/docs/covid-19-symptom-survey-request-for-data-access/)

# Medical Insurance Claims

Through partnership with Change Healthcare and others, we compute daily, county-level aggregate statistics from medical insurance claims covering over `r col("half of the US population")`
...
Despite challenges, these data have enormous potential for nowcasting: completely parallel data stream not subject to same issues as public health reporting 

<!--(limited testing capacity, reporting artifacts, retroactive re-definitions, misleading/misinterpreted aggregation rules)-->
...
*In an emergency, the systems you would want to use are the ones that are already up and running.*

<br>

The end of the pandemic is not the finish line. Let's take everything we've gained and build a `r col("permanent, nationally-coordinated effort on epidemic tracking and forecasting")`, and grow this technology to be as universally accepted and useful as weather forecasting is today
...
Why ask a proxy question (have people report on others)? Check out its Spearman correlations to COVID-19 case rates:
